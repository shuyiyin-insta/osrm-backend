...
using GeometryPoint = bg::model::d2::point_xy<std::int64_t>;
// Cell-contour outer rings are counter-clockwise and holes clockwise, matching GeoJSON's
// right-hand convention. Boost.Geometry's cartesian default is the opposite orientation.
using GeometryPolygon = bg::model::polygon<GeometryPoint, false, true>;
using GeometryMultiPolygon = bg::model::multi_polygon<GeometryPolygon>;

GeometryPolygon toGeometryPolygon(const osrm::engine::isochrone::GridPolygon &polygon)
{
    GeometryPolygon result;
    for (const auto point : polygon.outer)
        bg::append(result.outer(), GeometryPoint{point.x, point.y});
    result.inners().resize(polygon.holes.size());
    for (std::size_t hole_index = 0; hole_index < polygon.holes.size(); ++hole_index)
    {
        for (const auto point : polygon.holes[hole_index])
            bg::append(result.inners()[hole_index], GeometryPoint{point.x, point.y});
    }
    return result;
}
    ...
    const std::vector<osrm::engine::isochrone::GridPolygon> &polygons)
{
    ...
    for (const auto &polygon : polygons)
        result.push_back(toGeometryPolygon(polygon));
    return result;
}
...
BOOST_AUTO_TEST_CASE(produces_simple_oriented_rings_for_every_four_by_four_cell_configuration)
{
    ...
    for (std::size_t mask = 0; mask < (std::size_t{1} << (width * height)); ++mask)
    {
        BOOST_TEST_CONTEXT("mask=" << mask)
        {
            ...
            checkTopology(polygons);
            BOOST_CHECK_MESSAGE(bg::is_valid(toGeometryMultiPolygon(polygons)), "mask=" << mask);
